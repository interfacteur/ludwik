...
;var parametres = {
	...
	hauteur: .8, //cf. #figure dans jeu-puzzle.css et [.all-games, .lg-order] dans communs.css
...
;$(function () {
	...
	instancie.init = (function ii () {
		...
		commonLg.Sound.init(parametres.sons);
		...
		$(".form").css(commonLg.transition(game.delays[0], "opacity"));
...
/* WEBKIT
	to call SVG filters from CSS */
		...
		|| (instancie.webkit = commonLg.doNothing);


/* TACTILE - & MSIE?
	lack of resources for CSS transitions */
		commonLg.touch
		&& (commonLg.transition = function () {
			"use strict";
			return {}
		});
...
/* Class methods */
	...
	Piece.appreciate = function (cl) {
		...
		$stage.addClass(cl);
		setTimeout(function () {
			$svgBg[cl].css(commonLg.transition(game.delays[0]));
			$stage.removeClass(cl);
			setTimeout(function () {
				$svgBg[cl].removeAttr("style");
			}, game.delays[0] + 100);
		}, game.delays[1]);
		commonLg.sounds[cl].turnon();
		return true;
	}
...
/* Class methods for jQuery UI events manager */
	...
	Piece.toBrandTactile = commonLg.doNothing;
	commonLg.tactile(function () { //jQuery UI Touch Punch neutralizing mouseover event when dragging?
		"use strict";
...
/* Prototype */
	...
	Piece.prototype.wixer = commonLg.msieUp11 ?
	...
	Piece.prototype.finish = function (del, app, stop) {
		...
		game.win ++;
		$f.css(commonLg.transition(delay, "opacity"))
		...
		if (! stop) { //cf. Piece.prototype.stay()
			...
			this.$cloneImage.css(commonLg.transition(delay, "opacity"))
			.attr("class", "image");
	...
	Piece.prototype.againHelper = function (ev, fig) {
		...
		fig.css(commonLg.transition(0, "opacity"))
		.css("z-index", -1)
	...
	Piece.prototype.againStart = function (duel) {
		...
		this[! duel ? "$cloneImage" : "$cloneDuel"].css(commonLg.transition(0, "opacity"))
		.attr("class", "")
...
/* Conditionnal methods: puzzle with possible "hybridation" cf. data-dueljectif */
	...
	! game.duality
	&& (Piece.prototype.dualize = Piece.armor = Piece.prototype.armor = Piece.hybrid = commonLg.doNothing)
	...
	|| ((Piece.prototype.dualize = function () {
		...
		&& (Piece.prototype.stay = function (del) {
			...
			piece.$cloneDuel.css(commonLg.transition(delay, "opacity"))
			.attr("class", "image");
...
//Classical events ----------------------------------------------------------------------------
	instancie.classicalEvents = function () {
		...
		var clickToMove = ! commonLg.touch ? //added later
			[
			...
			:
			[commonLg.doNothing, commonLg.doNothing]; //TACTILE (nor MSIE): no pseudo drag and drop on click
