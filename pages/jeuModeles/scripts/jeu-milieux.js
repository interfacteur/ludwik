...
;$(function () {
	...
	instancie.init = (function ii () {
		...
		$(".lg-order").css(commonLg.transition(game.delays[5], ["background-color", "box-shadow"])); //game.delays[5]: cf. Piece.toTransit()
...
/* Class methods */
	...
	Piece.toZoom = function () { //later: zoom in game 1 and drag and drop in game 2 (starting was the opposite)
		...
		$zoom.movable = 
			commonLg.msieUp11 ? commonLg.doNothing //but ok with my prototype of magnify glass
			: function (x, y) {
				"use strict";
	...
	Piece.retroCaption = commonLg.doNothing; //third modification of storyboard
...
/* Class methods for jQuery UI events manager */
	...
	Piece.toTransit = function (clbck) { //added later
		...
		}, game.delays[5]); //game.delays[5]: cf. $(".lg-order").css(commonLg.transition()
	}
...
/* Prototype */
	...
	Piece.prototype.toFinish = function () {
		...
		$f.css(commonLg.transition(game.delays[0], "opacity"))
		.addClass("dragDropped");

		this.$cloneGroup.attr("class", "g");

		this.$cloneImage.css(commonLg.transition(game.delays[0], "opacity"))
		.attr("class", "image");
...
/* final definition of toBrandTactile() is asynchronous */
	...
	instancie.events3review = function () { //classical events for last review
		...
		commonLg.touch //TO DO: test on iPad (and Android)
		&& ($puzzle.stroking = function (ze) {
			"use strict";
			ze.preventDefault();
